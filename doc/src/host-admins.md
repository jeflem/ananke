...
They originate from setting up public test systems for the Ananke project and, thus, focus on hardware and network features available to the project team during time of development.
...
Ananke is mainly developed and tested on [Debian GNU/Linux](https://debian.org), but should work on all other major Linux distributions, too.
...
If you choose Debian for the host machine, follow the usual [install instructions](https://wiki.debian.org/DebianInstall). In some cases you might be interested in a [cross-install](https://www2.htw-dresden.de/~fjeme691/flemming/blog/cross_install.html).
...
See [`sshd_config` man page](https://manpages.debian.org/trixie/openssh-server/sshd_config.5.en.html) for details.
...
```
and then run `sudo systemd-tmpfiles --create`. See [Leap-16: openssh misses /run/utmp?](https://forums.opensuse.org/t/leap-16-openssh-misses-run-utmp/190495) for some background on this problem.

#### Fail2ban

To prevent brute-force attacks against SSH, install and enable ['Fail2ban'](https://www.fail2ban.org):
```
...
To restrict SSH access to certain IP ranges replace `tcp dport 986 accept`by `ip saddr {12.34.56.0/8, 123.45.0.0/16} tcp dport 986 accept`.
...
To check for modifications to your system you may use [TripWire](https://github.com/Tripwire/tripwire-open-source) or [AIDE](https://sourceforge.net/projects/aide/). To install AIDE run
```
...
-/var/www/html/
```
You may adjust this to your needs, see [`aide.conf` manpage](https://manpages.debian.org/trixie/aide/aide.conf.5.en.html).
...
We may use [one-size-fits-all `audit.rules`](https://github.com/Neo23x0/auditd) or more individual auditing rules like
```
