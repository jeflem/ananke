...
async def nbgrader_post_auth(authenticator: LTI13Authenticator, handler: LTI13CallbackHandler, authentication: dict) -> bool:
    ...
    if is_instructor and not grader_exists:
        ...
        os.system(f'runuser -u {grader_user} -- jupyter labextension disable --level=user @jupyter/nbgrader:formgrader')
        os.system(f'runuser -u {grader_user} -- jupyter labextension enable --level=user @jupyter/nbgrader:formgrader')
        os.system(f'runuser -u {grader_user} -- jupyter labextension disable --level=user @jupyter/nbgrader:assignment-list')
        os.system(f'runuser -u {grader_user} -- jupyter labextension disable --level=user @jupyter/nbgrader:create-assignment')
        os.system(f'runuser -u {grader_user} -- jupyter labextension enable --level=user @jupyter/nbgrader:create-assignment')
        os.system(f'runuser -u {grader_user} -- jupyter labextension disable --level=user @jupyter/nbgrader:validate-assignment')
