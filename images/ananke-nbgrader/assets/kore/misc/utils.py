...
def get_course_list(autogenerated_file_path: str, subset: Subset = Subset.ALL) -> Response:
    ...
    # Extend Subset.ACTIVE list with Subset.ALL list.
    ...
    try:
        from_course_names = generate_unique_course_names(active_paths=active_course_paths, backed_up_paths=backed_up_course_paths)
        to_course_names = generate_unique_course_names(active_paths=active_course_paths)
    except (FileNotFoundError, PermissionError, CalledProcessError, OSError) as e:
        return Response(response=json.dumps({'message': type(e).__name__}), status=500)

    # TODO: This has to be extended to represent FROM and TO with own names and paths. This has to be altered in TypeScript to.
    course_list = {
        'message': 'List of courses successfully retrieved.',
        'from_names': from_course_names,
        'from_paths': active_course_paths + backed_up_course_paths,
        'to_names': to_course_names,
        'to_paths': active_course_paths
    }
    logging.info(f'Generated course list: {course_list}')
